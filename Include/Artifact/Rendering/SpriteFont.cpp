...
namespace Artifact 
{
    SpriteFont::SpriteFont(const char* font, int size, char cs, char ce) 
    {
        ...
        for (i = 0; i < _regLength; i++) {
            ...
            _glyphs[i].uvRect = Rectangle(
                glm::vec2((float)glyphRects[i].x / (float)bestWidth,
        ...
        _glyphs[_regLength].uvRect = Rectangle(glm::vec2(0, 0), (float)rs / (float)bestWidth, 
    ...
    void SpriteFont::draw(SpriteBatch& batch, const char* s, glm::vec2 position, glm::vec2 scaling, 
                          float depth, Color tint, EJustification just /* = Justification::LEFT */)
    {
        glm::vec2 tp = position;
        // Apply justification
        if (just == EJustification::Middle)
        {
            tp.x -= measure(s).x * scaling.x / 2;
        } 
        else if (just == EJustification::Right)
        {
